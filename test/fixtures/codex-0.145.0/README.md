...
shape. The fixture deliberately retains an unfamiliar notification so Unknown
Event behavior stays covered.
...
| --- | --- |
| Captured live prompt, command, resource, duration, Unknown Event, history-only attribution, evaluation isolation, export | `replays the captured 0.145.0 code-review fixture through the black-box boundary` |
...
| Failed and cancelled turns | `reports failed and cancelled Skill Run outcomes...` |
| Reconnect recovery and unrecoverable gaps | `reconnects, recovers all available item history...`; `marks a recovered terminal run incomplete...`; `reports an Incomplete Trace when history recovery fails` |
| Isolated Obligation and Conformance Evaluation Runs | `compiles source-linked Obligations in an isolated Evaluation Run`; `evaluates every evaluable Obligation...` |
...
Evaluation responses and disconnect timing are intentionally scripted. They
are assertions about Tracer behavior, not claims that Codex emitted those
synthetic values in the manual run.

...
- Added focused projection-layout, session-snapshot, browser-shell, accessibility-semantics, and local-asset tests. Automated browser connection was unavailable in the implementation environment, so interaction verification remains covered by executable browser assets and server/DOM-surface tests rather than a live browser session.
